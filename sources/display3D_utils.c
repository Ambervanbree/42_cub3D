...
/*   Updated: 2022/04/06 15:13:01 by cproesch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	txt->strct.img = mlx_xpm_file_to_image(game->mlx,
			filename, &txt->width, &txt->height);
	if (!txt->strct.img)
		return (0);
	txt->strct.addr = mlx_get_data_addr(txt->strct.img,
			&txt->strct.bpp, &txt->strct.line_len, &txt->strct.end);
	return (1);
}
...
int	get_pixel_color(t_game *game, t_text text, int y, int x)
{
	...
	texture_x = (int)((game->threed_hit[x]
				- floor(game->threed_hit[x])) * (double)text.width);
